...
const architectureLayers = [
  {
    ...
    description: 'Hard real-time kernels, interrupt handling, and deterministic scheduling that guarantee bounded latency in safety-critical environments.',
  },
  {
    title: 'Grid Operating Layer',
    project: 'GridOS',
    description: 'Digital command surface for observability, coordination, and closed-loop control of smart grids and DER fleets.',
  },
  {
    title: 'AI Orchestration Layer',
    project: 'NeuralBridge',
    description: 'Middleware that connects human intent, large language models, and physical actuators while preserving deterministic guarantees.',
  },
  {
    title: 'Autonomous & Sensing Layer',
    project: 'Robotics & LiDAR Fusion',
    description: 'Perception pipelines and embodied intelligence that translate sensor data into verifiable physical actions.',
  },
];
...
const flagshipInitiatives = [
  {
    ...
    summary: 'AI-native middleware for human-to-model orchestration in safety-critical physics-informed environments.',
  },
  {
    title: 'GridOS',
    href: 'https://github.com/iceccarelli/GridOS',
    summary: 'Control-oriented operating surface for smart-grid intelligence, DER coordination, and real-time observability.',
  },
  {
    title: 'DERIM',
    href: 'https://github.com/iceccarelli/derim-middleware',
    summary: 'Distributed energy resource intelligence middleware focused on verifiable coordination and grid-aware execution.',
  },
  {
    title: 'Robot LiDAR Fusion',
    href: 'https://github.com/iceccarelli/robot-lidar-fusion',
    summary: 'Real-time perception and sensor fusion stack bridging software intelligence with physical autonomy.',
  },
];
...
// ====================== ORIGINAL VISUALIZERS (defined once) ======================
function GridLoadFrequencySimulator() {
  ...
  useEffect(() => {
    ...
    let animationFrame: number;
    let time = 0;
    const resize = () => { canvas.width = canvas.offsetWidth; canvas.height = 110; };
    window.addEventListener('resize', resize); resize();
    const draw = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.strokeStyle = 'rgba(52, 211, 153, 0.1)'; ctx.lineWidth = 1;
      for (let x = 25; x < canvas.width; x += 25) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, canvas.height); ctx.stroke(); }
      for (let y = 20; y < canvas.height; y += 20) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(canvas.width, y); ctx.stroke(); }
      ctx.strokeStyle = '#7dd3fc'; ctx.lineWidth = 3; ctx.shadowBlur = 12; ctx.shadowColor = '#38bdf8';
      ctx.beginPath();
      for (let x = 0; x < canvas.width; x += 2) {
      ...
      ctx.stroke();
      ctx.strokeStyle = 'rgba(52, 211, 153, 0.8)'; ctx.lineWidth = 2; ctx.shadowBlur = 8; ctx.shadowColor = '#34d399';
      ctx.beginPath();
      for (let x = 0; x < canvas.width; x += 2) {
      ...
      ctx.stroke();
      ctx.fillStyle = '#f0fdf4'; ctx.shadowBlur = 0;
      for (let i = 0; i < 10; i++) {
...
function NeuralBridgeActivityMap() {
  ...
  useEffect(() => {
    ...
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
    let animationFrame: number; let time = 0;
    const nodes = [{ x: 0.2, y: 0.3 }, { x: 0.5, y: 0.6 }, { x: 0.8, y: 0.4 }, { x: 0.35, y: 0.7 }, { x: 0.65, y: 0.25 }, { x: 0.15, y: 0.55 }, { x: 0.85, y: 0.65 }];
    const edges = [[0,1],[1,2],[2,3],[3,0],[1,4],[4,5],[5,6],[6,0],[2,6]];
    const particles = edges.map(([startIdx, endIdx]) => ({ startIdx, endIdx, progress: Math.random(), speed: 0.005 + Math.random() * 0.01 }));
    const resize = () => { canvas.width = canvas.offsetWidth; canvas.height = 110; };
    window.addEventListener('resize', resize); resize();
    const draw = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.strokeStyle = 'rgba(125, 211, 252, 0.1)'; ctx.lineWidth = 0.5;
      for (let x = 20; x < canvas.width; x += 20) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, canvas.height); ctx.stroke(); }
      for (let y = 20; y < canvas.height; y += 20) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(canvas.width, y); ctx.stroke(); }
      const nodePositions = nodes.map(n => ({ x: n.x * canvas.width, y: n.y * (canvas.height - 20) + 10 }));
      ctx.shadowBlur = 8; ctx.shadowColor = '#38bdf8';
      edges.forEach(([startIdx, endIdx]) => {
        const start = nodePositions[startIdx]; const end = nodePositions[endIdx];
        const gradient = ctx.createLinearGradient(start.x, start.y, end.x, end.y);
        gradient.addColorStop(0, 'rgba(125, 211, 252, 0.6)'); gradient.addColorStop(1, 'rgba(52, 211, 153, 0.6)');
        ctx.beginPath(); ctx.moveTo(start.x, start.y); ctx.lineTo(end.x, end.y); ctx.strokeStyle = gradient; ctx.lineWidth = 1.5; ctx.stroke();
      });
      ctx.shadowBlur = 12; ctx.shadowColor = '#7dd3fc';
      nodePositions.forEach(pos => {
        ctx.beginPath(); ctx.arc(pos.x, pos.y, 4, 0, Math.PI * 2); ctx.fillStyle = '#f8fafc'; ctx.fill(); ctx.strokeStyle = '#38bdf8'; ctx.lineWidth = 2; ctx.stroke();
      });
      ctx.shadowBlur = 0;
      particles.forEach(p => {
        const start = nodePositions[p.startIdx]; const end = nodePositions[p.endIdx];
        const currentX = start.x + (end.x - start.x) * p.progress;
        const currentY = start.y + (end.y - start.y) * p.progress;
        ctx.beginPath(); ctx.arc(currentX, currentY, 2.5, 0, Math.PI * 2); ctx.fillStyle = '#34d399'; ctx.fill();
        p.progress += p.speed; if (p.progress > 1) p.progress = 0;
      });
      time += 1; animationFrame = requestAnimationFrame(draw);
    };
    draw();
    return () => { window.removeEventListener('resize', resize); cancelAnimationFrame(animationFrame); };
  }, []);
...
function DERMCoordinationField() {
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    let animationFrame: number; let time = 0;
    const assets = Array.from({ length: 25 }, () => ({ x: Math.random(), y: Math.random(), vx: 0, vy: 0 }));
    const anchors = [{ x: 0.25, y: 0.5 }, { x: 0.75, y: 0.5 }];
    const resize = () => { canvas.width = canvas.offsetWidth; canvas.height = 110; };
    window.addEventListener('resize', resize); resize();
    const updateAssets = () => {
      assets.forEach(asset => {
        ...
        fx += (nearestAnchor.x - asset.x) * 0.005; fy += (nearestAnchor.y - asset.y) * 0.005;
        assets.forEach(other => {
          if (other === asset) return;
          const dx = asset.x - other.x; const dy = asset.y - other.y; const dist = Math.hypot(dx, dy);
          if (dist < 0.1 && dist > 0) { fx += (dx / dist) * 0.01; fy += (dy / dist) * 0.01; }
        });
        if (asset.x < 0.05) fx += 0.01; if (asset.x > 0.95) fx -= 0.01;
        if (asset.y < 0.05) fy += 0.01; if (asset.y > 0.95) fy -= 0.01;
        asset.vx = (asset.vx + fx) * 0.9; asset.vy = (asset.vy + fy) * 0.9;
        asset.x += asset.vx * 0.8; asset.y += asset.vy * 0.8;
      });
    };
    const draw = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.strokeStyle = 'rgba(52, 211, 153, 0.08)'; ctx.lineWidth = 1;
      for (let x = 25; x < canvas.width; x += 25) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, canvas.height); ctx.stroke(); }
      for (let y = 20; y < canvas.height; y += 20) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(canvas.width, y); ctx.stroke(); }
      ctx.shadowBlur = 15; ctx.shadowColor = '#34d399';
      anchors.forEach(anchor => {
        const pulse = 1 + Math.sin(time * 0.1) * 0.1;
        ctx.beginPath(); ctx.arc(anchor.x * canvas.width, anchor.y * canvas.height, 8 * pulse, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(52, 211, 153, 0.2)'; ctx.fill();
        ctx.strokeStyle = '#34d399'; ctx.lineWidth = 2; ctx.stroke();
      });
      ctx.shadowBlur = 8; ctx.shadowColor = '#7dd3fc'; ctx.lineWidth = 0.5;
      assets.forEach(asset => {
        let nearestAnchor = anchors[0];
        let minDist = Math.hypot(asset.x - nearestAnchor.x, asset.y - nearestAnchor.y);
        if (Math.hypot(asset.x - anchors[1].x, asset.y - anchors[1].y) < minDist) nearestAnchor = anchors[1];
        if (minDist < 0.3) {
          ...
          ctx.strokeStyle = 'rgba(125, 211, 252, 0.4)'; ctx.stroke();
        }
      });
      ctx.shadowBlur = 0;
      assets.forEach(asset => {
        ctx.beginPath(); ctx.arc(asset.x * canvas.width, asset.y * canvas.height, 3, 0, Math.PI * 2);
        ctx.fillStyle = '#f8fafc'; ctx.fill();
        ctx.strokeStyle = '#38bdf8'; ctx.lineWidth = 1.5; ctx.stroke();
      });
      updateAssets(); time += 1; animationFrame = requestAnimationFrame(draw);
    };
    draw();
    return () => { window.removeEventListener('resize', resize); cancelAnimationFrame(animationFrame); };
  }, []);
  return <canvas ref={canvasRef} className="system-waveform" aria-label="DER coordination field for distributed energy resources" />;
}
...
function LidarPointCloudVisualizer() {
  ...
  useEffect(() => {
    ...
    const ctx = canvas.getContext('2d');
    if (!ctx) return;
    let animationFrame: number; let angle = 0;
    const numPoints = 180;
    const points = Array.from({ length: numPoints }, (_, i) => {
      const a = (i / numPoints) * Math.PI * 2;
      return { angle: a, distance: 30 + Math.random() * 28, height: Math.sin(a * 3) * 15 + 50 };
    });
    const resize = () => { canvas.width = canvas.offsetWidth; canvas.height = 110; };
    window.addEventListener('resize', resize); resize();
    const draw = () => {
      ...
      for (let i = 0; i <= 12; i++) {
        ...
        ctx.beginPath(); ctx.arc(canvas.width / 2, canvas.height / 2, rad, 0, Math.PI * 2); ctx.stroke();
      }
      for (let i = 0; i < 12; i++) {
        const a = (i / 12) * Math.PI * 2;
        ctx.beginPath();
        ctx.moveTo(canvas.width / 2, canvas.height / 2);
        ctx.lineTo(canvas.width / 2 + Math.cos(a) * canvas.width * 0.45, canvas.height / 2 + Math.sin(a) * canvas.height * 0.45);
        ctx.stroke();
      }
      ...
      ctx.beginPath();
      ctx.moveTo(canvas.width / 2, canvas.height / 2);
      ctx.lineTo(canvas.width / 2 + Math.cos(angle) * canvas.width * 0.45, canvas.height / 2 + Math.sin(angle) * canvas.height * 0.45);
      ctx.strokeStyle = '#34d399'; ctx.lineWidth = 1.5; ctx.shadowBlur = 0; ctx.stroke();
      angle += 0.032;
      animationFrame = requestAnimationFrame(draw);
    };
    draw();
  ...
  return <canvas ref={canvasRef} className="system-waveform" aria-label="Simulated LiDAR point cloud scan" />;
}
...
function ControlSystemStepResponse() {
  ...
  useEffect(() => {
    ...
    if (!ctx) return;
    let animationFrame: number; let time = 0;
    const setpoint = 0.72; let response = 0.1;
    ...
    const updateResponse = () => {
      ...
      history.shift(); history.push(response);
    };
    const draw = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      ctx.strokeStyle = 'rgba(125, 211, 252, 0.1)'; ctx.lineWidth = 1;
      for (let x = 25; x < canvas.width; x += 25) { ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, canvas.height); ctx.stroke(); }
      for (let y = 20; y < canvas.height; y += 20) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(canvas.width, y); ctx.stroke(); }
      ctx.beginPath();
      ctx.moveTo(0, canvas.height - setpoint * canvas.height);
      ctx.lineTo(canvas.width, canvas.height - setpoint * canvas.height);
      ctx.strokeStyle = 'rgba(52, 211, 153, 0.4)'; ctx.lineWidth = 2; ctx.setLineDash([5, 3]); ctx.stroke(); ctx.setLineDash([]);
      ctx.beginPath();
      ...
      ctx.strokeStyle = '#7dd3fc'; ctx.lineWidth = 3; ctx.shadowBlur = 12; ctx.shadowColor = '#38bdf8'; ctx.stroke();
      ctx.beginPath();
      for (let i = 0; i < history.length; i++) {
        const x = (i / (history.length - 1)) * canvas.width;
      ...
      ctx.strokeStyle = 'rgba(52, 211, 153, 0.8)'; ctx.lineWidth = 2; ctx.shadowBlur = 8; ctx.shadowColor = '#34d399'; ctx.stroke();
      updateResponse(); time += 1; animationFrame = requestAnimationFrame(draw);
    };
    draw();
    return () => { window.removeEventListener('resize', resize); cancelAnimationFrame(animationFrame); };
  }, []);
  return <canvas ref={canvasRef} className="system-waveform" aria-label="Control system step response visualization" />;
}

// ====================== ROTATOR (7 visualizers) ======================
const visualizers = [
...
export default function Home() {
  ...
  useEffect(() => {
    const loadWidget = (ref: HTMLDivElement | null, scriptSource: string, config: Record<string, unknown>) => {
      if (!ref) return;
    ...
    loadWidget(marketOverviewRef.current, 'https://s3.tradingview.com/external-embedding/embed-widget-market-overview.js', {
      colorTheme: 'dark',
      dateRange: '12M',
      showChart: true,
      locale: 'en',
    ...
    const fetchSignals = async () => {
      try {
        ...
        if (!cancelled && Array.isArray(githubData)) {
          ...
          if (!cancelled) setRepoCards([...matched, ...remainder].slice(0, 6));
        }
        ...
        if (!cancelled) {
          setLastSync(`Last refreshed ${new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' }).format(new Date())}`);
        }
      } catch {
        if (!cancelled) setLastSync('Live sources temporarily unavailable — curated CPS signals still visible.');
      }
    };
    ...
    <main className="portfolio-shell">
      ...
      <section className="section-shell hero-section" id="top">
        <div className="hero-grid">
          <div className="hero-copy">
            ...
            <p>This portfolio is a living engineering record — connecting live market intelligence, flagship repositories, research trajectory, and implementation signals to demonstrate a coherent CPS execution path.</p>
            <div className="hero-actions">
              <a className="primary-button" href="#live-intelligence">Enter the Live Intelligence Hub</a>
              <a className="secondary-button" href="#flagship-systems">Explore Flagship Systems</a>
              <a className="secondary-button" href="https://github.com/iceccarelli" target="_blank" rel="noreferrer">GitHub Proof of Work</a>
            </div>
          </div>

          <aside className="glass-panel spotlight-border hero-panel">
            <div className="hero-portrait-shell">
              <img src="https://raw.githubusercontent.com/iceccarelli/vincenzo-grimaldi-portfolio/main/Vincenzo_Grimaldi_footer_picture_website.jpg" alt="Vincenzo Grimaldi" className="hero-portrait" />
            </div>
            ...
            <h2>Engineering deterministic systems that connect AI Driven Infrastructure and Automate Verifiable Executions.</h2>
      ...
      <section className="section-shell content-section" id="about">
        ...
        <div className="two-column-layout">
          <div className="glass-panel immersive-card">
            <p>I do not treat AI, software, infrastructure, energy, and robotics as isolated domains. I integrate them into deterministic physics-informed systems where every layer — from RTOS scheduling to AI orchestration — is verifiable and safety-critical.</p>
            <p>This site is structured to move visitors from high-level systems thinking → technical substance → live operational signals → immediate action.</p>
          </div>
      ...
      <section className="section-shell content-section" id="physics-informed">
        <div className="glass-panel cta-panel spotlight-border">
          ...
          <div className="two-column-layout">
            <div>
              <p className="section-intro">Traditional ML can violate conservation laws. Physics-Informed Neural Networks (PINNs) embed governing equations directly into the loss function.</p>
      ...
      <section className="section-shell content-section" id="agentic-systems">
        <div className="glass-panel cta-panel spotlight-border">
          ...
          <p className="section-intro">MARL with physics-informed rewards enabling transactive energy markets and verifiable DER negotiation.</p>
        </div>
      </section>
        ...
        <div className="card-grid two-up">
          {flagshipInitiatives.map((initiative) => (
            <article className="glass-panel immersive-card" key={initiative.title}>
              ...
              <a className="text-link" href={initiative.href} target="_blank" rel="noreferrer">View repository →</a>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="section-shell content-section">
        <div className="glass-panel cta-panel spotlight-border">
          ...
          <p>OT/IT infrastructure hardening, predictive maintenance, KRITIS-compliant cybersecurity and deterministic control for critical rail energy systems.</p>
        </div>
      </section>
        ...
        <div className="card-grid two-up">
          {phdApplications.map((application) => (
            <article className="glass-panel immersive-card" key={application.title}>
              ...
              <a className="text-link" href={application.href} target="_blank" rel="noreferrer">Open application listing →</a>
      ...
      <section className="section-shell content-section" id="connect">
        <div className="glass-panel cta-panel spotlight-border">
          <div>
            ...
            <p>Whether you are exploring AI-native middleware, smart-grid operating systems, embedded control platforms, robotics, or large-scale research collaboration — this portfolio is structured to make technical value visible.</p>
          </div>
          <div className="hero-actions">
            <a className="primary-button" href="https://github.com/iceccarelli" target="_blank" rel="noreferrer">GitHub Profile</a>
            <a className="secondary-button" href="#ecosystem">Trusted Ecosystem</a>
            <a className="secondary-button" href="mailto:vincenzo@grimaldi.engineering">Email — vincenzo@grimaldi.engineering</a>
          </div>
        </div>
      </section>
    </main>
